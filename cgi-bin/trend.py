...
print('<html lang="ru_RU"><head><meta charset="UTF-8">')
...
for s in sortBy:
    ...
    print( \
        '<td><table border="1">' +
        inTAG('tr', ''.join(list(
            map(lambda c: inTAG('th' c, param=['style="text-align: center;"',]), col + ['BID %%', 'OFFER%%', 'Тенденция'])
        )))
    )
    tot = {
        'BIDDEPTHT': 0,
    ...
    for i in range(15):
        ...
        print('<tr>%s<td>%s %%</td><td>%s %%</td><td>%s</td></tr>' %(
                ''.join(list(map(lambda c: '<td>%s</td>' %str(sortBy[s][i][c]), col))),
                bid, offer,
                getTrend(bid, offer)
            )
        )
    ...
    print('<tr><td colspan=3>ИТОГО</td><td></td><td>%d</td><td></td><td>%d</td><td>%s %%</td><td>%s %%</td><td>%s</td></tr>' %(
            tot['BIDDEPTHT'], tot['OFFERDEPTHT'],
            bid, offer,
            getTrend(bid, offer)
        )
    )
